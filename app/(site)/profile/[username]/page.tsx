...
export default async function UserProfilePage({
  ...
  const profileUser = rawUser as unknown as {
    ...
    location?: string; avatarColor?: string; createdAt?: Date;
  };
  ...
  const [rawPosts, totalCount] = await Promise.all([
    Post.find({ author: username }).sort({ createdAt: -1 }).skip(skip).limit(POSTS_PER_PAGE).lean(),
  ...
  const posts: PostData[] = rawPosts.map((p) => {
    const post = p as unknown as {
      ...
      likes?: string[]; comments?: unknown[]; createdAt?: Date;
    };
    return {
      _id: post._id.toString(), title: post.title, content: post.content,
      createdAt: post.createdAt?.toISOString() ?? new Date().toISOString(),
      likesCount: post.likes?.length ?? 0, commentsCount: post.comments?.length ?? 0,
    };
  });
