...
export default function AdminUsersClient({ users: initialUsers }: { users: UserData[] }) {
  ...
  function handleToggleBan() {
    ...
    startTransition(async () => {
      ...
      setUsers((prev) =>
        prev.map((u) => (u.id === banTarget.id ? { ...u, isBanned: newBanState } : u))
      );
      setBanTarget(null);
      router.refresh();
    });
    ...
    <div style={{ padding: "32px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#9ca3af", margin: 0 }}>{users.length} total site users</p>
        </div>
        <button
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.id} style={{ borderBottom: "1px solid #f5f5f5" }}>
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{
                      ...
                      background: user.isBanned ? "#9ca3af" : "#f97316",
                ...
                <td style={{ padding: "14px 16px" }}>
                  {user.isBanned ? (
                    <span style={{ fontSize: 11, fontWeight: 700, color: "#dc2626", background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 50, padding: "3px 10px" }}>
                      Banned
                    </span>
                  ) : (
                    <span style={{ fontSize: 11, fontWeight: 700, color: "#16a34a", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 50, padding: "3px 10px" }}>
                      Active
                    </span>
                  )}
                </td>
                ...
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <button
                      type="button"
                      onClick={() => { setBanError(""); setBanTarget(user); }}
                      style={{
                        padding: "5px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600,
                        cursor: "pointer", fontFamily: "inherit",
                        border: user.isBanned ? "1px solid #16a34a" : "1px solid #f59e0b",
                        background: "#fff",
                        color: user.isBanned ? "#16a34a" : "#f59e0b",
                      }}
                    >
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: "28px 32px", width: 380 }}>
            <h3 style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 800 }}>
              {banTarget.isBanned ? "Unban this user?" : "Ban this user?"}
            </h3>
            <p style={{ fontSize: 13, color: "#6b7280", margin: "0 0 16px" }}>
              {banTarget.isBanned
                ? <>Restoring access — <strong>{banTarget.username}</strong> will be able to log in again.</>
                : <><strong>{banTarget.username}</strong> will no longer be able to log in. Account and posts stay intact.</>}
            </p>
            {banError && (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: "28px 32px", width: 360 }}>
            ...
            <p style={{ fontSize: 13, color: "#6b7280", margin: "0 0 6px" }}>
              This will permanently delete <strong>{deleteTarget.username}</strong> and all their posts.
            </p>
